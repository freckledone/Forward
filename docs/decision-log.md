...
- **Impact:** Verified by device-signed build: `com.apple.developer.healthkit` is present in the signed entitlements and granted by the provisioning profile, so automatic signing registered the capability with no manual portal step.
  ...
  - **Corrected 2026-09-09:** the original cut shipped only `NSHealthUpdateUsageDescription`, on the reasoning that declaring a read purpose the app never uses would be misleading. App Store validation rejects that: the `com.apple.developer.healthkit` entitlement alone triggers the requirement for `NSHealthShareUsageDescription`, regardless of whether `read:` is ever populated. The string added says outright that Further reads nothing and that Apple requires the key anyway — honest to both the reviewer and the user, since iOS never surfaces it (no read authorization is ever requested).
...
- **Decision:** On Home, the workout card is **inert** and a single Start button begins the workout. Long-press opens a context menu with Edit and Delete, the latter behind an alert. There is no swipe action.
- **Amended 2026-09-09, twice.** An earlier cut made the *whole card* the start target. Reverted — a session can't be un-started, so that action shouldn't fire from anywhere a thumb lands while scrolling. The replacement, a solid accent disc, was then toned down: unambiguous but far too loud on a list where every card already carries its own muscle tint. The button now reads through **shape** — a lightly tinted fill inside a defined accent ring — rather than through saturation. A swipe-to-edit action was also dropped: a swipe and a context menu offering the same Edit is two hidden gestures for one command.
- **Why:** Editing a workout is rare next to running one, so it shouldn't own the card's tap. But starting is not undoable, which rules out the largest possible target: the fix is a clearer button, not a bigger one.
...
- **Date:** 2026-09-09
- **Impact:** `WorkoutCard` holds no button of its own any more — `StartButton` is the only control, which removes the nested-button problem D-056 worked around. `onOpen` is gone.
  - **The press animation is feedback, not a gate.** A first attempt delayed the start by 180ms so a confirmation beat could play; that made the one control that begins a workout feel unresponsive. Trading responsiveness for polish is the wrong way round here, so the animation now fires alongside the start rather than before it, and the press-down scale carries the "it was hit" signal on its own. Delete is now reachable from Home as well as the editor, both behind an alert (D-066).
