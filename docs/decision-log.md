...
- **Decision:** On Home, the workout card is **inert** and a single Start button begins the workout. Editing moves to a trailing swipe action; long-press opens a context menu with Edit and Delete, the latter behind an alert.
- **Amended 2026-09-09:** an earlier cut made the *whole card* the start target. Reverted — a session can't be un-started, so that action shouldn't fire from anywhere a thumb lands while scrolling. The button instead became unmistakable: solid accent fill with a white glyph rather than a 12%-tinted icon, which read as an ornament on a card rather than as the control.
...
- **Impact:** `WorkoutCard` holds no button of its own any more — `StartButton` is the only control, which removes the nested-button problem D-056 worked around. `onOpen` is gone.
  - Pressing runs a 180ms confirmation beat (circle swells, glyph bounces, light haptic) before the workout screen takes over, so the press reads as *started* rather than the screen merely changing. 180ms sits under the threshold where a transition begins to feel sluggish. A guard makes a double-tap start one session, not two. Delete is now reachable from Home as well as the editor, both behind an alert (D-066).
